...
public class CreateGroupServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(user != null){
            String nameAssociation = request.getParameter("nameAssociation");
            ...
            users.add(user);
            association.setUserList(users);
            association.setAssociationRequestList(new ArrayList<AssociationRequest>());
            List<Association> member = user.getAssociationList();
            member.add(association);
            user.setAssociationList(member);
            List<Association> admin = user.getAssociationList1();
            admin.add(association);
            user.setAssociationList1(admin);
            
            associationFacade.create(association);
